...
SCREEN_WIDTH = 1150
SCREEN_HEIGHT = 650
gameDisplay = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_WIDTH),0,0)
...
def gameLoop():
    ...
    random_apple_x = int(round(random.randrange(2 * block_size, SCREEN_WIDTH - 2 * block_size) / 20) * 20)
    random_apple_y = int(round(random.randrange(2 * block_size, SCREEN_HEIGHT - 2 * block_size) / 20) * 20)
    ...
    while not gameExit:
        ...
        if random_apple_x - block_size < lead_x < random_apple_x + 40 and lead_y >  random_apple_y - block_size and lead_y< random_apple_y +40:
            random_apple_x = int(round(random.randrange(0, SCREEN_WIDTH - block_size) / block_size) * block_size)
            random_apple_y = int(round(random.randrange(0, SCREEN_HEIGHT - block_size) / block_size) * block_size)
            score += 1
            snakeLength += 1
            print("Yum Yum Yum")
            eating_apple_sound = pygame.mixer.Sound("eating_apple.wav")
            pygame.mixer.Sound.play(eating_apple_sound)
        if not((0 <= lead_x <= SCREEN_WIDTH) and (0 <= lead_y <= SCREEN_HEIGHT)):
            gameOver = True
